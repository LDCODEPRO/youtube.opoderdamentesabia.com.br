...
def api_resumo(request: Request, force: bool = False, _=Depends(require_auth)):
    ...
    for v in dados["pipeline"].get("videos", []):
        ...
        if v.get("capa_url"):
            ...
            if v["crivo"]:
                crivos_recentes.append({"tipo": "capa", "codigo": v["codigo"],
                                        "aprovada": v["crivo"].get("aprovada"),
                                        "resumo": v["crivo"].get("resumo", ""),
                                        "quando": v["crivo"].get("quando", ""),
                                        "checks": v["crivo"].get("checks", [])})
        v["portao"] = pode_publicar(v, dados["decisoes"])
        cv = v["portao"].get("crivo") or {}
        if cv.get("checks"):
            crivos_recentes.append({"tipo": "video", "codigo": v["codigo"],
                                    "aprovada": cv.get("aprovado"),
                                    "resumo": cv.get("resumo", ""),
                                    "quando": dados.get("coletado_em", ""),
                                    "checks": cv.get("checks", [])})
    dados["crivos_recentes"] = crivos_recentes
    try:
        with open(os.path.join(BASE_DIR, "data", "agenda_canal.json"), encoding="utf-8") as f:
            dados["agenda_canal"] = json.load(f)  # quantos vídeos e quando — palavra do Pesquisador
    except Exception:
        dados["agenda_canal"] = {}
    disp = {}
    dbase = os.path.join(BASE_DIR, "agente", "equipe", "disparador")
    for chave, arq in (("grupos", "grupos_nicho.json"), ("mensagens", "mensagens.json"),
                       ("fila", "fila_disparo.json"), ("nichos", "nichos.json")):
        try:
            with open(os.path.join(dbase, arq), encoding="utf-8") as f:
                disp[chave] = json.load(f)
        except Exception:
            disp[chave] = {}
    # diário de campanha: cada linha = um disparo REAL feito (grupo, quando, mensagem, reação)
    disparados = []
    try:
        with open(os.path.join(dbase, "diario_campanha.jsonl"), encoding="utf-8") as f:
            disparados = [json.loads(ln) for ln in f if ln.strip()]
    except Exception:
        pass
    disp["disparados"] = disparados[-200:]
    dados["disparador"] = disp  # o braço de divulgação em grupos do nicho
    return JSONResponse(dados)


@app.get("/api/estudo")
def api_estudo(request: Request, _=Depends(require_auth)):
    """O estudo completo da semana (markdown) para a rota de leitura do painel."""
    caminho = os.path.join(BASE_DIR, "ESTUDO_YOUTUBE_2026.md")
    try:
        with open(caminho, encoding="utf-8") as f:
            return {"ok": True, "markdown": f.read()}
    except Exception:
        return {"ok": False, "markdown": "# Estudo ainda não sincronizado nesta máquina."}
...
def _decisao_permitida(item: str, decisao: str) -> bool:
    ...
    if item.startswith("pauta_"):
        return decisao in {"escolhida", "adiada", "descartada"}
    if item.startswith("crivo_"):
        return decisao in {"concordo", "discordo"}
    if item == "receita_semana":
        return decisao in {"aprovada", "revisar"}
...
class AgendarIn(BaseModel):
    codigo: str
    quando: str  # "AAAA-MM-DD HH:MM" no horário de Brasília


@app.post("/api/agendar")
def api_agendar(body: AgendarIn, request: Request, _=Depends(require_auth)):
    """O Diretor marca QUANDO o vídeo vai ao ar. Fica no pipeline e vira carimbo no Cofre."""
    quando = body.quando.strip()
    if not re.fullmatch(r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}", quando):
        raise HTTPException(status_code=422,
                            detail="Formato do horário: AAAA-MM-DD HH:MM (horário de Brasília)")
    pipe_path = os.path.join(BASE_DIR, "pipeline.json")
    try:
        with open(pipe_path, encoding="utf-8") as f:
            pipe = json.load(f)
    except Exception:
        raise HTTPException(status_code=500, detail="pipeline.json ilegível")
    alvo = next((v for v in pipe.get("videos", []) if v.get("codigo") == body.codigo), None)
    if not alvo:
        raise HTTPException(status_code=404, detail="Vídeo não está na fila")
    alvo["publicar_em"] = quando
    tmp = pipe_path + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(pipe, f, ensure_ascii=False, indent=2)
    os.replace(tmp, pipe_path)
    grava_decisao(f"agenda_{body.codigo}", "definida", quando)
    return {"ok": True, "publicar_em": quando}


# ── Disparador: buscar grupos + preparar disparo ─────────────────────────────
DISP_DIR = os.path.join(BASE_DIR, "agente", "equipe", "disparador")


def _disp_le(arq, default):
    try:
        with open(os.path.join(DISP_DIR, arq), encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return default


def _disp_grava(arq, dados):
    from dados import _grava_json_atomico
    _grava_json_atomico(os.path.join(DISP_DIR, arq), dados)


class BuscaGruposIn(BaseModel):
    nicho: str


@app.post("/api/disparador/buscar")
def api_disp_buscar(body: BuscaGruposIn, request: Request, _=Depends(require_auth)):
    """Diretor digita um nicho → o servidor varre diretórios e traz grupos REAIS (best-effort).
    O que achar entra no catálogo; o nicho fica registrado para o catalogador varrer a fundo."""
    nicho = (body.nicho or "").strip()
    if len(nicho) < 3:
        raise HTTPException(status_code=422, detail="Diga um nicho com pelo menos 3 letras")
    try:
        from buscar_grupos import buscar
        achados = buscar(nicho)
    except Exception as e:
        achados = []
        print("[buscar_grupos]", e)
    cat = _disp_le("grupos_nicho.json", {"grupos": []})
    existentes = {g.get("link") for g in cat.get("grupos", [])}
    novos = [g for g in achados if g.get("link") and g["link"] not in existentes]
    if novos:
        cat.setdefault("grupos", []).extend(novos)
        cat["total"] = len(cat["grupos"])
        cat["por_plataforma"] = {p: sum(1 for g in cat["grupos"] if g["plataforma"] == p)
                                 for p in ("whatsapp", "telegram", "facebook")}
        from datetime import datetime, timezone
        cat["atualizado_em"] = datetime.now(timezone.utc).isoformat()
        _disp_grava("grupos_nicho.json", cat)
    # registra o pedido (o catalogador — workflow — varre a fundo depois)
    from datetime import datetime, timezone
    ped = _disp_le("buscas_pedidas.json", {"pedidos": []})
    ped.setdefault("pedidos", []).append(
        {"nicho": nicho, "quando": datetime.now(timezone.utc).isoformat(),
         "achou_na_hora": len(novos)})
    _disp_grava("buscas_pedidas.json", ped)
    # o nicho pedido entra na lista-mãe → passa a ser varrido pela caça diária
    nj = _disp_le("nichos.json", {"nichos": [], "descobertos": []})
    ja = {n.lower() for n in nj.get("nichos", []) + nj.get("descobertos", [])}
    if nicho.lower() not in ja:
        nj.setdefault("descobertos", []).append(nicho)
        nj["atualizado_em"] = datetime.now(timezone.utc).isoformat()
        _disp_grava("nichos.json", nj)
    return {"ok": True, "nicho": nicho, "novos": len(novos), "novos_grupos": novos,
            "total_catalogo": len(cat.get("grupos", []))}


class LevaIn(BaseModel):
    mensagem_idx: int
    grupos: list[str] = []
    nota: str | None = None


@app.post("/api/disparador/leva")
def api_disp_leva(body: LevaIn, request: Request, _=Depends(require_auth)):
    """O Diretor prepara e APROVA um disparo (carimbo): escolhe a mensagem e os grupos.
    A leva fica aprovada; o envio REAL é pelo navegador do PC (lei anti-ban), nunca do servidor."""
    from datetime import datetime, timezone
    msgs = _disp_le("mensagens.json", {}).get("mensagens", [])
    if not (0 <= body.mensagem_idx < len(msgs)):
        raise HTTPException(status_code=422, detail="Mensagem inexistente")
    fila = _disp_le("fila_disparo.json", {"levas": []})
    leva = {
        "id": "leva-" + str(len(fila.get("levas", [])) + 1),
        "criada_em": datetime.now(timezone.utc).isoformat(),
        "aprovada": True,  # carimbo do Diretor
        "mensagem_idx": body.mensagem_idx,
        "mensagem_titulo": msgs[body.mensagem_idx].get("titulo", ""),
        "mensagem_texto": msgs[body.mensagem_idx].get("texto", ""),
        "grupos": body.grupos,
        "nota": (body.nota or "")[:300],
        "estado": "aprovada — aguardando envio pelo navegador do PC",
    }
    fila.setdefault("levas", []).append(leva)
    _disp_grava("fila_disparo.json", fila)
    return {"ok": True, "leva": leva}
